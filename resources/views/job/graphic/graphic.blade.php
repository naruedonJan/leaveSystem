...
<div class="panel panel-primary">
    <div class="panel-heading"><h4>งานกราฟฟิค</h4></div>
    <div class="panel-body">
        <form action="{{ url('/job/graphic') }}" method="GET">
            @csrf
            <div class="row">
        ...
        <table class="table table-hover table-striped">
            <thead class="thead-dark">
                <tr>
                    ...
                    <th class="text-center" scope="col" width="20%">ชื่องาน</th>
                    <th class="text-center" scope="col" width="10%">ประเภท</th>
                    <th class="text-center" scope="col" width="20%">รายละเอียด</th>
                    <th class="text-center" scope="col" width="20%">ที่อยู่ไฟล์</th>
            ...
            <tbody>
                ...
                @endswitch
                        ...
                        @endif
                        <td> {{ $job->pic_detail }} </td>
                        <td> <a href="{{ $job->pic_address }}" target="_blank"> {{ $job->pic_address }} </a></td>
                        @if(Auth::user()->emp_id == $job->id)
                        <td class="text-center">
                        <form action="{{ url('/job/graphic') }}" method="GET">
                            @csrf
                            <input type="hidden" name="del_id" value="{{ $job->PicID }}">
