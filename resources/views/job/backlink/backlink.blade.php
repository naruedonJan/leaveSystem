...
<div class="panel panel-primary">
    <div class="panel-heading"><h4>งานแบ็คลิงค์</h4></div>
    <div class="panel-body">
        <form action="{{ url('/job/backlink') }}" method="GET">
            @csrf
            <div class="row">
        ...
        <table class="table table-hover table-striped">
            <thead class="thead-dark">
                <tr>
                    ...
                    <th class="text-center" scope="col" width="20%">ชื่อเว็บ</th>
                    <th class="text-center" scope="col" width="20%">กลุ่ม</th>
            ...
            <tbody>
                ...
                @endswitch
                        ...
                        <td>{{ $job->Weblink }}</td>
                        <td> {{ $job->groupf }} </td>
                        <td class="text-center"> <a href="{{ $job->Webposted }}" target="_blank"> คลิก </a></td>
                        <td> {{ $job->comment }} </td>
                        @if(Auth::user()->emp_id == $job->id)
                        <td class="text-center">
                        <form action="{{ url('/job/backlink') }}" method="GET">
                            @csrf
                            <input type="hidden" name="del_id" value="{{ $job->WID }}">
